...
namespace isim
{
    ...
    bool should_cull_transparent_backface(const Ray &ray, const Object *object,
    ...
    Color Camera::trace_ray(Ray ray, Color backgroundColor, Color scene_ambient,
                            ...
                            bool calculate_alpha_shadows = true) const
    {
        ...
        for (const auto *ob : objects)
        {
            ...
            if (is_colliding < closest_distance && is_colliding > 0)
            {
                ...
                if (should_cull_transparent_backface(ray, ob, hit_point,
                                                     hit_props))
                {
                    continue;
                }
        ...
        if (closest_object)
        {
            ...
            Vector3 geometric_normal =
                closest_object->getNormalAt(collision_point);
            ...
            if (properties.ambient_color.a == 0 && object_ior <= 1.0)
            {
                ...
                return trace_ray(new_ray, backgroundColor, scene_ambient,
                                 objects, lights, current_ior, depth - 1);
            }

            // compute light
            int final_r = clamp_color(
                properties.ambient_color.r * DEFAULT_AMBIENT_LIGHT.color.r
                * DEFAULT_AMBIENT_LIGHT.intensity / 255.0);
            int final_g = clamp_color(
                properties.ambient_color.g * DEFAULT_AMBIENT_LIGHT.color.g
                * DEFAULT_AMBIENT_LIGHT.intensity / 255.0);
            int final_b = clamp_color(
                properties.ambient_color.b * DEFAULT_AMBIENT_LIGHT.color.b
                * DEFAULT_AMBIENT_LIGHT.intensity / 255.0);

            for (const auto *light : lights)
            {
                Vector3 light_dir =
                    Vector3(light->position.x - collision_point.x,
                ...
                float intensity =
                    std::max(0.0f,
                             static_cast<float>(
                                 normal_at_collision.dotProduct(light_dir)));
                ...
                float specular_intensity = pow(
                    std::max(S.normalize().dotProduct(light_dir), 0.0f), ns);
                    ...
                    for (int b = 0; b < max_transparent_bounces; ++b)
                    {
                        ...
                        for (const auto *ob : objects)
                        {
                            ...
                            if (t > 0 && t < closest_shadow_t
                                && t < distance_to_light)
                            {
                                Point3 shadow_hit_candidate =
                                    Point3(shadow_ray.origin.x
                                               + shadow_ray.direction.x * t,
                                           shadow_ray.origin.y
                                               + shadow_ray.direction.y * t,
                                           shadow_ray.origin.z
                                               + shadow_ray.direction.z * t);

                                Texture_properties shadow_candidate_props =
                                    ob->getTextureCoordinatesAt(
                                        shadow_hit_candidate);
                ...
                Color light_color = light->renderLight(
                    properties, intensity, specular_intensity,
                ...
                || is_refractive_material)
            {
                ...
                reflected_color =
                    trace_ray(reflected_ray, backgroundColor, scene_ambient,
                              objects, lights, current_ior, depth - 1);
                has_reflected_color = true;

                const float ks = std::clamp(
                    static_cast<float>(closest_object->material->properties.ks),
                    0.0f, 1.0f);
            ...
            if (is_refractive_material)
            {
                ...
                const float cos_theta_i =
                    std::clamp(static_cast<float>(-ray.direction.dotProduct(
                                   normal_at_collision)),
                               0.0f, 1.0f);
                const float k =
                ...
                if (k >= 0.0f)
                {
                    refract_dir = (ray.direction * eta
                                   + normal_at_collision
                                       * (eta * cos_theta_i - std::sqrt(k)));
                }
                    ...
                    || refract_dir.z != -1)
                {
                    ...
                    Point3 corrected_collision_point = Point3(
                        collision_point.x - normal_at_collision.x * 0.001f,
                        collision_point.y - normal_at_collision.y * 0.001f,
                        collision_point.z - normal_at_collision.z * 0.001f);
                    ...
                    const float transparency = std::clamp(
                        static_cast<float>(255 - properties.ambient_color.a)
                            / 255.0f,
                        0.0f, 1.0f);
                    ...
                    float reflection_weight =
                        has_reflected_color ? fresnel : 0.0f;
                    float refraction_weight = has_refracted_color
                        ? (1.0f - fresnel) * transparency
                        : 0.0f;
                    float base_weight = std::max(
                        0.0f, 1.0f - reflection_weight - refraction_weight);
    ...
    Image Camera::render(const std::vector<const Object *> &objects,
                         ...
                         size_t res_y, Color backgroundColor,
                         Color scene_ambient, bool ray_tracing) const
    {
        double aspect_ratio =
        ...
        for (size_t y = 0; y < res_y; y++)
        {
            for (size_t x = 0; x < res_x; x++)
            {
                ...
                Vector3 pixel_3d = centerVector + (forward * zmin)
                    + (right * px) + (true_up * py);
                ...
                Color pixel_color =
                    trace_ray(ray, backgroundColor, scene_ambient, objects,
                              lights, -1, MAX_REFLECTION_DEPTH);
                pixel_color.a = 255;
